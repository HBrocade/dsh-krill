...
**npm 的 `latest` dist-tag 在 dsh 家族上不可信。** `@deepseek-ai/dsh-base` 等包的 `latest`
指向 `0.0.1-rc.1`，而实际最新是 `0.1.0-rc.7`。任何版本检测都必须拉 `versions` 全量列表
自己做 semver 排序 —— 用 `npm view <pkg> version` 会把升级报成降级。
